...
程序不要求 LLM 凑齐固定数量，但现有 Tag 清洗仍保留 65 个内容 tags 的技术上限。质量词、固定角色、画师预设和画风不计入这个上限。
...
与自动 tags 模式不同，`无优化` 会完全按原样使用后面的提示词，不再拼接质量词、固定角色、画师预设或画风，也不执行内容 Tag 清理。
...
启用画师预设后，插件会先拼接当前画师预设 tags；当前启用的画风组会继续拼接在画师预设之后，用于填写媒介、上色和渲染 tags。画风组通过 `style_presets` 保存，并用 `active_style_preset` 切换。

`default_artist_tags` 仅在没有启用画师预设时作为备用画师 tags 使用。
