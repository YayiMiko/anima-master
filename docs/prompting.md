...
在描述前后附带 `--自由发挥`，插件会跳过现成 Tag 快路径，让 LLM 自主把简短主题发展成统一、完整的角色画面，并重点补充服装结构、材质纹样、配饰、姿态手势、前景互动、构图、光影和少量特效。该模式通常以 50–65 个互不重复的内容 Tag 为目标，背景仍保持在约 2–6 个有区分度的元素。

角色身份、人数、用户明确指定的主体、关键服装、动作、表情和道具仍然是硬约束。参数只对当前请求生效，也可以写成 `--自由拓展`、`--创意扩展` 或 `--creative`。

## 普通模式的场景类 Tag 门控

没有使用 `--自由发挥` 时，如果描述没有明确提到前景互动、构图或镜头、环境或背景、光影、特效或氛围中的任何一类，LLM 只生成主体关系、角色、服装、材质配饰、动作手势、神态和视线相关 Tag，不会自行添加简易背景、景深、镜头角度、光照、粒子或氛围词，也不会为了达到常规数量而补入这些内容。

只要描述明确提到上述任意一类，场景类门控便会整体打开。此后 LLM 可以根据主题需要补充全部场景类型，但不要求每类都生成，背景仍保持简洁。
...
自然语言主题通常生成 40-55 个内容 tags；简单表情包或头像可以使用 30-45 个，复杂服装或复杂构图可增加到约 60 个，硬上限为 65 个。质量词、固定角色、画师组和画风不计入这个上限。

模板会优先保留用户明确指定的主体、花卉、道具、动作、表情和镜头，并限制互相冲突的动作与神态。抽象主题至少保留两个直接可见的主题锚点，避免被常见礼服、花朵或场景素材取代。

提示词会限制高置信度同义词簇。删除重复词后，空出的内容额度会优先分配给动作与物件关系、服装结构、材质纹样、手部、前景互动、空间层次以及功能不同的光影。普通模式下，如果清理后内容过短，或一次清除了较多重复项，插件会进行一次定向补全；千代turbo的低 CFG Harness 不使用这次补全。
