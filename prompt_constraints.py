...
def build_constraint_plan_prompt(user_prompt: str, llm_content: str, fixed_character_name: str = "") -> str:
    """Build the LLM prompt that asks for a structured constraint plan.

    Args:
        user_prompt: User's original natural-language request.
        llm_content: Candidate tags produced by the prompt builder LLM.
        fixed_character_name: Name of the selected fixed character, if any.

    Returns:
        A prompt asking the LLM to return only JSON.
    """
    fixed_note = (
        f"The selected fixed character is {fixed_character_name}. "
        "If the user explicitly requests a body part, accessory, pose, or action, that explicit request overrides generic character-tag stripping."
...
def parse_constraint_plan(text: str) -> PromptConstraintPlan:
    """Parse a JSON constraint plan from LLM output.

    Args:
        text: Raw LLM output.

    Returns:
        Parsed and bounded constraint plan. Invalid output returns an empty plan.
    """
    ...
    has_constraints = bool(data.get("has_constraints")) and bool(priority_tags or remove_tags or max_content_tags)
    return PromptConstraintPlan(
...
def apply_prompt_constraints(content_tags: str, plan: PromptConstraintPlan | None = None) -> PromptConstraintResult:
    """Apply an LLM-proposed constraint plan to cleaned content tags.
    ...
    Returns:
        Reordered content tags with priority tags front-loaded and planned
        removals applied.
    """
    ...
    remove_keys = {normalize_tag_key(tag) for tag in plan.remove_tags if normalize_tag_key(tag)}
...
def _should_remove(key: str, remove_keys: set[str]) -> bool:
    if key in remove_keys:
        return True
    return any(remove_key and (key in remove_key or remove_key in key) for remove_key in remove_keys)
