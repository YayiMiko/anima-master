...
> **0.3.4：** 支持在聊天指令中按别名或明确数值临时指定分辨率；优化提示词同义词清理与定向细节补全，在控制重复 Tag 的同时保留画面层次和细节密度。
...
在生图描述前后附带 `--自由发挥`，会让聊天模型在保留角色身份和明确要求的前提下，自主发展统一主题并丰富服装、姿态、材质、配饰、前景、构图、光影和少量特效。该模式仍保持背景简洁，且只对本次请求生效。兼容参数：`--自由拓展`、`--创意扩展`、`--creative`。
