...
一个 **AstrBot** 插件，把聊天里的自然语言变成适合 **本地 ComfyUI / Anima 工作流** 的 Danbooru tags，并把生成结果发回聊天。它把「中文描述 → 提示词优化 → ComfyUI 出图 → 发回群里」整个流程做成一个 `/anm` 指令。
...
- **多人画面**：`/anm 多人` 生成 2–4 人画面，逐个分析身份、外观、服装、表情、动作与道具，并补全位置与互动关系。
- **标签直出**：`/anm 无优化` 跳过提示词优化，完全按你写的标签直接出图。
- **固定角色**：保存角色名到 tags，指令中命中名字时自动引用；未点名不会自动套用。
- **画师预设与画风**：独立保存并切换画师预设（`artist:` 串）与画风预设，也支持「不使用默认画风」。
- **联网校正角色 Tag**：少量联网搜索补充角色外观/服装参考，再用 Danbooru / Safebooru 角色分类校正，让角色外观更稳定。
...
2. 模型把它整理成 Danbooru tags；固定角色、画师预设、画风预设、质量词与负面词由插件在最终 Prompt 前后拼接。
...
| `/anm 创建画师预设 <名称>=<tags>` | 保存并启用画师预设（旧名「画师组」仍可用） |
| `/anm 追加画师预设 <名称>=<tags>` | 追加画师预设内容（旧名「画师组」仍可用） |
| `/anm 切换画师预设 <名称>` | 切换当前画师预设（旧名「画师组」仍可用） |
| `/anm 查看画师预设` | 列出画师预设（旧名「画师组」仍可用） |
| `/anm 删除画师预设 <名称>` | 删除画师预设（旧名「画师组」仍可用） |
| `/anm 帮助` | 查看指令表 |
...
之后在指令中明确提到角色名时才会拼接该角色 tags；没点名不会自动套用。已保存角色的固定 Tag 是判定外观身份的标准，即使 LLM 规划了冲突的发色、瞳色或种族特征，插件也会丢弃冲突外观，只保留可变部分。

### 画师与画风

启用画师预设后，先拼接当前画师预设的标签，再拼当前画风预设（用于媒介、上色、渲染）。`default_artist_tags` 仅在未启用画师预设时作为备用。某次不想用画师标签，可在指令里写：
...
`无优化` 完全按你给的提示词出图，不拼接质量词、固定角色、画师预设或画风预设，也不清理标签。
...
在「千代预设」中选择 `千代base`、`千代aesthetic` 或 `千代turbo`，会写入对应的千代画师预设并把「狐莉」加入固定角色（狐莉不是默认角色，只有点名才用）。
...
千代turbo 使用 `variants/turbo/workflows/comfyui_00051_api.json`，固定 10 步、`euler`、`simple`，并启用针对 CFG 1 的二次提示词约束。选择「未启用」会恢复启用前的值。
