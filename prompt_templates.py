...
- 抽象主题可以转化为可见符号，但必须保留至少两个直接表达原主题的视觉锚点，不要让自行补充的王座、礼服、花朵等常见素材取代主题。
...
- 常规输出 40-55 个互不重复、可见、可生成的内容 tags；复杂服装或复杂构图可增加到约 60 个，最多不得超过 65 个。已经是 tags 的输入不设最低数量。画面元素单薄时宁可保持简洁，也不要加入用户未要求的道具、鞋袜、花朵、建筑或表情。
